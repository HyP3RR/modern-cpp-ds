...
3. creating default deleter, and storing it's object incurs extra bytes penalty
as even empty classes have ->0 size.. can use EBO (empty base optimisation and
just inherit from it, and call the default deleter!) directly via this pointer!!
(no penalty size of class).. empty class  contains no non-static data mem
(directly/indirectly)
...
solve deleter while incorporating ebo, must include T* ptr inside the compressed
object, otherwise adding an object of deleter is not considered EBO.. thus, a
compressed obj of deleter and pointer , template specialised on basis of empty
lambda (ie is_empty), accordingly define it we utilise ebo in the compressed
object, since we don't define an obj of base member default_delete! while it is
not valid if we do T* ptr and define base obj of compressed_deleter in unique
pointer. (completely wrong design)
...
template <typename T, typename deleter,
          bool empty = std::is_empty<deleter>::value>
struct compressed_obj
    : private deleter { // inherit from stateless lambda or default_del
    T *ptr_;
    // dont declare obj of base, hence ebo valid.
    compressed_obj() = default;
    compressed_obj(T *obj) : ptr_(obj) {}
    compressed_obj(T *obj, deleter d)
        : deleter(d), ptr_(obj) {
    } // to conform to lambdas, no memory alloted, base initialised first.
    compressed_obj(compressed_obj<T, deleter> &&other) noexcept
        : ptr_(other.ptr_) {
        other.ptr_ = nullptr;
    }

    compressed_obj<T, deleter> &operator=(compressed_obj<T, deleter> &&other) {
        ptr_ = other.ptr_;
        other.ptr_ = nullptr;
        return *this;
    }
...
struct compressed_obj<T, deleter, false> {
    ...
    compressed_obj(compressed_obj<T, deleter> &&other) noexcept
        : ptr_(other.ptr_), custom_deleter_(other.custom_deleter_) {
        other.ptr_ = nullptr;
    }
...
template <typename T, typename deleter = default_delete<T>> class unique_ptr {
  public:
    unique_ptr(T *obj = nullptr)
        : obj_{obj} {
    } // T deduced when declaring class, T* thus compulsory for ptr type.
    ...
    operator=(const unique_ptr<T, deleter> &other) = delete;
    ...
    T *get() const noexcept { return obj_.ptr_; }
    ...
    void reset(T *p = nullptr) noexcept {
        ...
        obj_.ptr_ = p;
        if (old)
            call_deleter(old); // delete later to avoid leaking old, but need
                               // specialised overload to accept ptr
    }
...
int main() {
    // prat::unique_ptr<int, decltype(del)> ptr(new int,del); //calling delete
    // on stack alloted ptr is UB std::unique_ptr<int, decltype(del)> g(new int,
    // del);
    // std::unique_ptr<int, void(*)(int*)> ptr2(new int,func);  //decltype(func)
    // not a func ptr.

    // std::cout <<sizeof(prat::unique_ptr<int>) << " "
    // <<sizeof(std::unique_ptr<int>) <<"\n"; std::cout <<sizeof(ptr) <<" "
    // <<sizeof(g)  <<"\n"; std::cout
    // <<sizeof(prat::unique_ptr<int,void(*)(int*)>) << " "
    // <<sizeof(std::unique_ptr<int,void(*)(int*)>) <<"\n"; std::cout
    // <<sizeof(prat::unique_ptr<std::function<void(int*)>>) << " "
