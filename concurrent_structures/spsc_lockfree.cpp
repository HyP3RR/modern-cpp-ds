 ...
 **acquire release ensures the variable involved, no instr is reordered after
it.. so R and W are synchronised... how does this help for fences?
...
template <typename T> class SPSC {
  public:
    SPSC()
        : capacity_(1024),
          ring_(reinterpret_cast<T *>(::operator new(sizeof(T) * capacity_))),
          head_{}, tail_{} {}

    bool push(const T &val) {
        // tail only incremented by push thread since SPSC, so no need to load
        // it by seq cst... local variable usage is preferred to calculate sizes
        // etc, lesser memory cycles wasted...
        auto tail =
            tail_.load(std::memory_order_relaxed); // use local variables
                                                   // directly by loading
        auto head = head_.load(std::memory_order_acquire);
        if (tail - head != capacity_) {
            new (ring_ + tail % capacity_)
                T(val); // invoke copy constructor via placement new.
    ...
    bool pop(T &place) {
        // to ensure exception safety, user allocates memory to place popped
        // object in  directly via move.
        auto head = head_.load(std::memory_order_relaxed);
        auto tail = tail_.load(std::memory_order_acquire);
        if (head < tail) {
            new (&place)
                T(std::move(ring_[head % capacity_])); // no allocation, only
                                                       // move construction
            // via placement new.
            ring_[head % capacity_]
                .~T(); // destroy the obj to prevent memory leak.
    ...
    ~SPSC() {
        while (!empty()) {
            ring_[head_ % capacity_].~T(); // placement new usage should be
                                           // combined with destructor calling
            head_++;
        }
        ::operator delete(
            ring_); // since we did operator new, didnt make array via new.
    }
  ...
  private:
    ...
    alignas(std::hardware_destructive_interference_size)
        std::atomic<size_t> head_; // pop at head
    alignas(std::hardware_destructive_interference_size)
        std::atomic<size_t> tail_; // push at tail , align to prevent sharing of
                                   // cache line...
