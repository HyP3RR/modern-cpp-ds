...
Drawbacks:
1. if writing, we block consumer reading, even if theres no contention. -> can
use two mutex, one for producing and one for consuming, notify the respective
one.
2. this approach is blocking , ie threads can be suspended indefinitely if
another thread holds the lck
3. pace depends on scheduler! if scheduler takes away time slice for the locked
one, no one makes progress.

soln:
1. lockfree -> atleast one thread always make progress, even if others are
paused (no matter what)
...
template <typename T> class MPMC {
  public:
    ...
    void try_produce(T obj) {
        ...
        if (full())
            throw std::runtime_error("filled buffer");
    ...
    void produce(T obj) {
        ...
        full_check_.wait(uniq_lk,
                         [this] { return !full(); }); // wait till not full
        buffer_[mask(out_++)] = std::move(obj);
        empty_check_.notify_one();
        // if(++out_ == MAXSIZE) out_ = 0; dont wrap around!
    }
    ...
    std::shared_ptr<T> consume() {
        ...
        empty_check_.wait(uniq_lk, [this] {
            return !empty();
        }); // use unique lock with cv, wait to fill
        auto my_ptr = std::make_shared<T>(buffer_[mask(in_)]);
        in_++;
        full_check_.notify_one();
        // to establish exception guarantee, return pointer... copy return might
        // throw, hence data loss.
        return my_ptr;
    }
